...
export class ApplicationsService {
  ...
  async getApplications(query: GetApplicationQueryDto): Promise<{ data: PersonProfile[]; total: number}> {
    ...
    const data = await this.dataSource.query<PersonProfile[]>(
      `SELECT * FROM person_profile ${whereClause} ORDER BY created_at DESC LIMIT ${limitParameter} OFFSET ${offsetParameter}`, params,
    );

    const countResult = await this.dataSource.query<{count: string} []> (
      `SELECT COUNT(*) FROM person_profile ${whereClause}`, status ? [status] : []
    )

    return { data, total: parseInt(countResult[0].count, 10) };
  }

  async updateStatus (person_id: string, status: string): Promise<PersonProfile>{
    const updateQuery = await this.dataSource.query<PersonProfile[]>(
      'UPDATE person_profile SET application_status = $1 WHERE person_id = $2 RETURNING *', [status, person_id],
    );
